...
![whid.gif](https://github.com/lyb200/lua_nvim/blob/main/pic/whid.gif)

## 插件目录结构

我们的插件至少应该有两个目录：`plugin` 用于存放主文件，lua 目录放置整个基本代码。当然，如果需要的话，我们可以把所有代码都放在一个文件中，但是，请不要成为这样的人。因此，创建 `plugin/whid.vim` 和 `lua/whid.lua` 比较好。我们从这里开始：
